...
document.addEventListener('DOMContentLoaded', function() {
  document.getElementById('login-btn').addEventListener('click', tutorLogin);
  document.getElementById('login-email').addEventListener('keydown', function(e) { if (e.key === 'Enter') tutorLogin(); });
  document.getElementById('submit-btn').addEventListener('click', submitSummary);
  document.getElementById('logout-btn').addEventListener('click', tutorLogout);
  document.getElementById('tab-tutor').addEventListener('click', function() { switchTab('tutor', this); });
  document.getElementById('tab-owner').addEventListener('click', function() { switchTab('owner', this); });
});
...
function switchTab(tab, btn) {
  ...
  if (tab === 'tutor') {
    if (currentTutor) showView('view-tutor-form');
    else showView('view-tutor-login');
  } else {
    showView('view-owner-info');
  }
}
...
async function tutorLogin() {
  ...
  if (!sb) { errEl.textContent = 'Still loading, please try again in a moment.'; errEl.classList.add('show'); return; }
  btn.disabled = true;
...
async function submitSummary() {
  ...
  try {
    ...
    await fetch('/.netlify/functions/notify', {
      ...
      body: JSON.stringify({ id: summary.id, studentName: studentName, tutorName: tutorName, sessionDate: d, topics: topics, progress: progress, homework: homework, siteUrl: window.location.origin })
    });
